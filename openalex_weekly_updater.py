...
BATCH = 300          # DB upsert batch size
API_PAGE_SLEEP = 0.35  # seconds between OpenAlex pages (polite)
...
def process_window(conn, base_filter: str, extra_filters: dict, label: str, seen_ids: set):
    ...
    while True:
        ...
        with conn.cursor() as cur:
            for w in results:
                ...
                for pos, a in enumerate(w.get("authorships") or [], start=1):
                    ...
                    aid = author.get("id")
                    if aid:
                        execute_batch(cur, SQL_UPSERT_AUTHOR, [(aid, aid, author.get("display_name"), author.get("orcid"))], page_size=1)
                ...
                if len(work_rows) >= BATCH:
                    execute_batch(cur, SQL_UPSERT_WORKS, work_rows); work_rows.clear()
                if len(inst_rows) >= BATCH:
                    execute_batch(cur, SQL_UPSERT_INST, inst_rows); inst_rows.clear()
                if len(auth_rows) >= BATCH:
                    execute_batch(cur, SQL_UPSERT_AUTHORSHIP, auth_rows); auth_rows.clear()
        ...
        conn.commit()  # commit each page so progress is visible
        if not next_cursor:
            break
